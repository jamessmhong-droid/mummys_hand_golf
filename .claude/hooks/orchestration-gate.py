#!/usr/bin/env python3
# .claude/hooks/orchestration-gate.py — fable 강제 게이트 (PreToolUse, 레포 커밋용 클라우드 변형)
#
# 메인 에이전트가 한 턴(prompt_id)에 코드 파일을 2개까지만 직접 수정하게 허용하고,
# 3개째부터 차단하며 위임 지시를 돌려준다. 셸(Bash/PowerShell)을 통한 코드 파일 수정은 항상 차단.
# 서브에이전트(payload 에 agent_id/agent_type 존재)는 전부 통과 — 위임이 실행 경로다.
#
# 스위치 (로컬 변형과 다른 부분):
#   1) 환경변수 FABLE_GATE=off      → 통과 (클라우드/로컬 어디서든 끄는 스위치)
#   2) ~/.claude/.fable-state 존재  → 그 값을 따름 (로컬 PC의 fable on/off 연동)
#   3) 상태 파일 없음(클라우드 샌드박스) → 기본 ON
...
CODE_EXTS = {
    ...
    "zsh", "ps1", "psm1", "sql", "lua", "dart", "scala", "ex", "exs", "zig",
    "ipynb",
}
EXT_RE = "|".join(sorted(CODE_EXTS))

DELEGATE_HINT = (
    "일반 구현·수정·테스트·리뷰 → 실행 에이전트(Opus), "
...
def gate_enabled():
    if os.environ.get("FABLE_GATE", "").lower() == "off":
        return False
    try:
        return open(STATE_FILE).read().strip() == "on"
    except OSError:
        return True  # 상태 파일 없음(클라우드) → 기본 ON
...
def main():
    ...
    if tool in ("Bash", "PowerShell"):
        ...
        writes_cmdlet = re.search(
            r"\b(Set-Content|Out-File|Add-Content)\b[^|;]*\.(?:%s)\b" % EXT_RE,
            cmd, re.IGNORECASE,
        )
        touches_code = re.search(r"\.(?:%s)\b" % EXT_RE, cmd)
        if writes_redirect or writes_cmdlet or (writes_inplace and touches_code):
            deny(
                "[fable 게이트] 메인 에이전트는 셸 명령으로 코드 파일을 수정할 수 없습니다. "
    ...
    if not prompt:
        allow()  # 구버전(prompt_id 미지원)에서는 턴 리셋이 불가능하므로 통과
    ...
    deny(
        ...
        % (LIMIT, ", ".join(gate["files"]), DELEGATE_HINT, path)
    )
