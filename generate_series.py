...
<meta name="description" content="골프를 바꾼 핵심 연구 18선. 매주 한 편씩, 카드·근거·딥다이브를 한 페이지에서.">
...
body{background:var(--bg);color:var(--ink);font-family:'Pretendard','Apple SD Gothic Neo','Segoe UI',system-ui,sans-serif;line-height:1.6;-webkit-font-smoothing:antialiased}
.wrap{max-width:940px;margin:0 auto;padding:34px 20px 90px}
.hero{text-align:center;padding:6px 0 22px}
.logo{width:92px;height:auto;display:block;margin:0 auto 14px;filter:drop-shadow(0 12px 24px rgba(230,15,115,.4))}
.kicker{display:inline-block;background:var(--hot);color:#fff;font-size:12px;font-weight:800;letter-spacing:.13em;padding:6px 14px;border-radius:999px;margin-bottom:14px}
.hero h1{font-size:32px;font-weight:900;letter-spacing:-.02em;line-height:1.24}
.hero h1 span{color:var(--hot)}
.hero p{margin-top:12px;color:var(--muted);font-size:15px;max-width:580px;margin:12px auto 0}
.prog{max-width:560px;margin:22px auto 0}
.progbar{height:12px;background:var(--bg-soft);border:1px solid var(--line);border-radius:999px;overflow:hidden}
.progbar span{display:block;height:100%;background:var(--hot);border-radius:999px;transition:width .6s ease}
.proglabel{margin-top:8px;font-size:12.5px;font-weight:800;color:var(--muted)}

/* 리더(현재 선택 회차) */
.reader{margin:26px 0 8px;background:var(--white);border:1px solid var(--line);border-radius:24px;overflow:hidden;box-shadow:0 20px 50px -30px rgba(230,15,115,.55)}
.rhead{background:var(--ink);color:#fff;padding:22px 24px 20px;position:relative;overflow:hidden}
.rhead::after{content:"";position:absolute;right:-40px;top:-40px;width:170px;height:170px;background:radial-gradient(circle,rgba(255,30,136,.5),transparent 70%)}
.rhead .tag{display:inline-block;background:var(--hot);color:#fff;font-size:11.5px;font-weight:800;letter-spacing:.1em;padding:5px 12px;border-radius:999px}
.rhead .no{font-size:14px;font-weight:800;color:#FF7DB4;margin-left:8px}
.rhead h2{font-size:26px;font-weight:900;letter-spacing:-.02em;line-height:1.3;margin:12px 0 6px;position:relative}
.rhead .t{font-size:14px;color:#EBC9D9;font-weight:700;position:relative}
.tabs{display:flex;gap:6px;padding:14px 16px 0;flex-wrap:wrap}
.tab{flex:1;min-width:96px;text-align:center;cursor:pointer;font-size:13.5px;font-weight:800;color:var(--muted);background:var(--bg-soft);border:1px solid var(--line);border-radius:12px 12px 0 0;padding:12px 8px;transition:background .12s,color .12s}
.tab small{display:block;font-size:11px;font-weight:700;color:var(--muted);margin-top:2px}
.tab.cur{background:var(--hot);color:#fff}
.tab.cur small{color:#ffe3f0}
.framewrap{padding:0 16px 18px}
...
.sechead{font-size:14px;font-weight:900;color:var(--hot-deep);text-transform:uppercase;letter-spacing:.05em;margin:30px 0 14px;display:flex;align-items:center;gap:9px}
.sechead::before{content:"";width:10px;height:10px;background:var(--hot);border-radius:3px}
.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}
.ep{text-align:left;background:var(--white);border:1px solid var(--line);border-radius:16px;padding:16px 16px 15px;position:relative;transition:transform .14s ease,box-shadow .14s ease;font-family:inherit}
.ep.open{cursor:pointer}
.ep.open:hover{transform:translateY(-3px);box-shadow:0 14px 30px -16px rgba(230,15,115,.5)}
.ep.sel{border-color:var(--hot);box-shadow:0 0 0 2px var(--hot) inset}
.ep .epno{font-size:22px;font-weight:900;color:var(--hot)}
.ep .ephook{font-size:13.5px;font-weight:800;line-height:1.45;margin-top:6px;color:var(--ink);display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
.ep .epgo{font-size:11.5px;font-weight:800;color:var(--hot-deep);margin-top:10px;display:inline-block}
.ep.lock{background:var(--bg-soft);border-style:dashed;cursor:default}
.ep.lock .epno{color:var(--line)}
.ep.lock .ephook{color:var(--muted)}
.ep .lockrow{font-size:11.5px;font-weight:800;color:var(--muted);margin-top:10px}
.ep .badge{position:absolute;top:12px;right:12px;font-size:16px}
...
<header class="hero">
  <img class="logo" src="assets/mascot.png" alt="마미손 골프 마스코트">
  <span class="kicker">WEEKLY SERIES · 매주 1편</span>
  <h1>골프를 바꾼 <span>핵심 연구 18선</span><br>매주 한 편씩</h1>
  <p>골프는 18홀, 그래서 핵심 연구도 18선. 매주 한 편씩 <b>카드·근거·딥다이브</b>를 이 페이지 안에서 그대로 읽습니다. 공개된 편만 열립니다.</p>
...
<section class="reader" id="reader" hidden>
  <div class="rhead">
    ...
    <h2 id="rhook"></h2><div class="t" id="rtitle"></div>
  </div>
  <div class="tabs">
    <div class="tab cur" data-fmt="v1">V1 카드<small>스와이프 요약</small></div>
    <div class="tab" data-fmt="v2">V2 근거<small>전문가 자료집</small></div>
    <div class="tab" data-fmt="v3">V3 딥다이브<small>논문 해부</small></div>
  </div>
  <div class="framewrap"><iframe id="frame" title="회차 콘텐츠" sandbox="allow-same-origin" scrolling="yes"></iframe></div>
  <div class="hint">이 페이지 안에서 세 가지 버전을 모두 볼 수 있어요. 위 탭으로 전환하세요.</div>
</section>

<div class="countdown" id="countdown"></div>

<div class="sechead">전체 회차</div>
...
function selectEp(n){
  ...
  document.getElementById('rtitle').textContent = m.part + ' · ' + m.title;
  document.getElementById('rtag').textContent = (n===UNLOCKED)?'이번 주 공개':'다시 보기';
  var tabs = document.querySelectorAll('.tab');
  tabs.forEach(function(t){ t.classList.toggle('cur', t.dataset.fmt==='v1'); });
  document.querySelectorAll('.ep').forEach(function(el){
    el.classList.toggle('sel', el.dataset.n==String(n)); });
  load();
}
document.querySelectorAll('.tab').forEach(function(t){
  t.addEventListener('click', function(){
    fmt = t.dataset.fmt;
    document.querySelectorAll('.tab').forEach(function(x){ x.classList.toggle('cur', x===t); });
    load();
  });
});
...
for(var i=0;i<TOTAL;i++){
  var m = META[i], n = m.n, no = String(n).padStart(2,'0');
  if(m.open){
    html += '<button class="ep open" data-n="'+n+'"><span class="badge">🩷</span>'+
      '<div class="epno">'+no+'</div><div class="ephook">'+m.hook+'</div>'+
      '<span class="epgo">이 편 열기 →</span></button>';
  } else {
    html += '<div class="ep lock"><span class="badge">🔒</span>'+
      '<div class="epno">'+no+'</div><div class="ephook">공개 예정</div>'+
      '<div class="lockrow">'+m.date+' 공개</div></div>';
  }
}
g.innerHTML = html;
document.querySelectorAll('.ep.open').forEach(function(b){
  b.addEventListener('click', function(){
    selectEp(parseInt(b.dataset.n,10));
